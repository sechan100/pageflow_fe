...
import { useRootAuth } from "./useRootAuth";
...
export const useAuth = () => {
  const { isAuthenticated } = useRootAuth(); // 인증 상태 참조
  ...
  const ensureToken: () => Promise<string> = async () => {
    ...
    if(isRefreshRequired){
      const request = new AsyncRequestBuilder(DefaultAxiosFactory.createDefaultAxiosInstance(), null);
